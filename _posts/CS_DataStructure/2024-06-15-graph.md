...
<span style="background-color: #fff5b1">희소 그래프에 가까울수록 인접리스트가 적합, 조밀 그래프에 가까울 수록 인접 행렬이 적합</span>
